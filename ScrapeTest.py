...
topMoviesChart = requests.get('https://www.imdb.com/chart/moviemeter/?ref_=nv_mv_mpm').text

#activate beautiful soup to filter the data to be readable
soup = BeautifulSoup(topMoviesChart, 'lxml')

#target the area where the desired data is located
chartContent = soup.find('tbody', class_='lister-list')

#pull all rows containing the desired data
chartRows = chartContent.find_all('tr')
...
for chartRow in chartRows:
    ...
    titles = chartRow.find_all('td', class_='titleColumn')
    ...
    ratings = chartRow.find_all('td', class_='ratingColumn imdbRating')
